...
class paging {
    ...
    function __construct($seo=0, $first="First", $last="Last", $next="Next", $previous="Previous") {
    ...
    function processPaging($rowsPerPage, $pageID) {
        ...
        if ($record >= $rowsPerPage)
            $record = ceil($record / $rowsPerPage); //ceil method give below round figure.
        else
            $record = 1;
    ...
    function myRecordCount($query) {
        ...
        unset($rs); //To uninitialize the value
        return $rsCount;
    }
